...
// src/index.js
var ORIGINS = ["https://susieanddima.com", "https://www.susieanddima.com", "http://localhost:8790", "http://127.0.0.1:8790"];
var src_default = {
  async fetch(req, env) {
    ...
    if (req.method === "GET") {
      const list = await env.BUCKET.list({ prefix: "guestbook/", limit: 1e3 });
    ...
    if (req.method === "POST") {
      ...
      await env.BUCKET.put(
        `guestbook/${ts}-${key}.json`,
        JSON.stringify({ key, name, message, ts }),
...
// .wrangler/tmp/bundle-lOoTND/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-lOoTND/middleware-loader.entry.ts
